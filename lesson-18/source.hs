...
getEmptyDrawers :: [Int] -> [Maybe Organ] -> [Int]
getEmptyDrawers ids organs =
  map (\(id, _) -> id) emptys
  where pairs = zip ids organs
        emptys = filter (\(id, x) -> x == Nothing) pairs

getNothingId :: (Int, Maybe Organ) -> Maybe Int
getNothingId (id, Nothing) = Just id
getNothingId (_, Just _) = Nothing
