...
median :: (Real a, Ord a) => [a] -> a
median xs = medianValue
  where sorted = sort xs
        medianIndex = (length xs) `div` 2
        medianValue = head (drop medianIndex xs)

medianTS :: (Real a) => TS a -> Maybe a
medianTS (TS _ []) = Nothing
medianTS (TS _ values) = if all (== Nothing) values
                       then Nothing
                       else Just m
  where justVals = filter isJust values
        cleanVals = map fromJust justVals
